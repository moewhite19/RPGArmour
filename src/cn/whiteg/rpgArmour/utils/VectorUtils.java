...
public class VectorUtils {
     ...
     * @return 从起点到终点的仰角
     */
    public static float getLocPirch(Location loc1,Location loc2) {
        double dx = loc1.getX() - loc2.getX();
        double dz = loc1.getZ() - loc2.getZ();
        double dis = Math.sqrt(dx * dx + dz * dz);
        double dy = loc1.getY() - loc2.getY();
        return (float) Math.toDegrees(Math.atan2(dy,dis));
    }
     ...
     */
    public static float checkViewCone(Location player,Location monster,float ConeAngle) {
        Vector u = monster.toVector().subtract(player.toVector());
        Vector v = viewVector(player);
