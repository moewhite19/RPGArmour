...
public class EnderBow extends CustItem_CustModle implements Listener {
    ...
    public void onHit(final ProjectileHitEvent event) {
        ...
        if (shooter instanceof LivingEntity) who = (LivingEntity) shooter;
        else return;
        if (!event.getEntity().getScoreboardTags().contains(endTag)) return;
        if (event.getHitEntity() != null && (event.getHitEntity() instanceof Mob || event.getHitEntity() instanceof HumanEntity)){
            ents[0] = who;
